...
arguments:
  - shellQuote: false
    valueFrom: |-
      ...
      echo -e "BioProject ID\tRelated PubMed IDs\tRelated PubMed Titles\tRelated SRA Files\tPlatform\tNumber of Files\tTotal Size (GB)" > "$(inputs.output_file)"
      ...
      for bp_id in $bp_ids; do
        ...
        echo -e "$bp_id\t$pubmed_ids\t$pubmed_titles\t$sra_accession\t$sra_platform\t$num_files\t$total_size_gb" >> "$(inputs.output_file)"
      done       
inputs:
  species_name:
    type: string
